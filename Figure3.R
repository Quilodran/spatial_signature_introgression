...
source("Rcustom/filled.contour4.R") #modified from http://wiki.cbr.washington.edu/qerm/sites/qerm/images/1/16/filled.contour4.R
mycolors<-colorRampPalette(c("black", "cyan", "aquamarine4", "darkolivegreen","burlywood", "gold1","darkred"))
rotate <- function(x) t(apply(x, 2, rev))
coor <- read.table("Rcustom/genes1.sam", skip=1)
xy<-data.frame(x= coor[1:25,4], y= coor[1:25,5])


pdf("Figure3.pdf", paper="special", family = "Helvetica", onefile = FALSE, width= 6.83, height= 6.83, colormodel="rgb",  pointsize=12)
...
introI<-sapply(1:24, function(i){
	k=(24*(NC-1)+i)
	propI <- sapply(0:24, function(j){ round(1-inv[k,5+j], 3 )})
	return(propI)
})

introL<-sapply(1:24, function(i){
	k=(24*(NC-1)+i)
	propL <- sapply(0:24, function(j){ round(loc[k,5+j], 3 )})
	return(propL)
})

matI<-t(as.matrix(introI[which((xy[,1]-xy[,2])==0),]))[24:1,]
matL<-t(as.matrix(introL[which((xy[,1]-xy[,2])==0),]))[24:1,]


filled.contour4(rotate(matI),  col=mycolors(100), levels=seq(0, 1, length=101), axes=F)
axis(2, at=seq(0,1, length=4), labels=seq(0,0.06, length=4), tck=-0.01,  padj=0.5, cex.axis=0.8, mgp=c(3, 0.5, 0), lwd.ticks=1, las=2)
...
text(x=-0.35, y=0.5, "Invasive", cex=1, font=2, srt=90)
text(x=-0.53, y=0.5, "Whole area", cex=1, font=2, srt=90)
par(xpd=F)
filled.contour4(rotate(matL),  col=mycolors(100), levels=seq(0, 1, length=101), axes=F)
axis(2, at=seq(0,1, length=4), labels=seq(0,0.06, length=4), tck=-0.01,  padj=0.5, cex.axis=0.8, mgp=c(3, 0.5, 0), lwd.ticks=1, las=2)

par(xpd=NA)
text(x=-0.35, y=0.50, "Local", cex=1, font=2, srt=90)
text(x=-0.53, y=0.50, "Whole area", cex=1, font=2, srt=90)
par(xpd=F)
...
par(xpd=NA)
text(x=-0.35, y=0.50, "Invasive", cex=1, font=2, srt=90)
text(x=-0.53, y=0.50, "Restricted area", cex=1, font=2, srt=90)
par(xpd=F)
...
par(xpd=NA)
text(x=-0.35, y=0.50, "Invasive 1", cex=1, font=2, srt=90)
text(x=-0.53, y=0.50, "Two invasions", cex=1, font=2, srt=90)
par(xpd=F)
...
par(xpd=NA)
text(x=-0.35, y=0.50, "Invasive 2", cex=1, font=2, srt=90)
text(x=-0.53, y=0.50, "Two invasions", cex=1, font=2, srt=90)
par(xpd=F)
...
text(x=-5.78, y=3.4, expression(paste("Interbreeding rate ", (gamma))), cex=1.5, srt=90, font=2)
